...
public class OracleDB implements IDatabase
{
    ...
    public  void connect(String sid)
    {
        try {
            ...
            initBySid(sid);
        } catch (ClassNotFoundException e) {Log.getInstance().error(e);}
        try {
            String url="jdbc:oracle:thin:@"+config.get(sid+"IP")+":"+config.get(sid+"PORT")+":"+config.get(sid+"Server"); //orcl为数据库的SID
            con = DriverManager.getConnection(url, this.getUsername(), this.getPassword());
            stmt = con.createStatement();
        } catch(SQLException e) {Log.getInstance().error("连接数据库失败: "+url,e);}
    }
